 ...
 * che modella una factory per SubsequenceCombiner, che a sua volta modella un
 * trasformatore da liste a liste, che funziona spezzando la lista in input in
 * vari pezzi, ottenendo un elemento in uscita per ognuno di essi.
 * 
 ...
 * punteggio: - implementazione del quinto metodo della factory (ossia, a scelta
 * se ne realizzino 4, il quinto è opzionale) - concisione del codice e
 * rimozione di tutte le ripetizioni con eventuale uso appropriato di pattern
 *
 * Si tolga il commento dal metodo init.
 * 
 * Indicazioni di punteggio: - correttezza della parte obbligatoria: 10 punti -
 * correttezza della parte opzionale: 3 punti - qualità della soluzione
 * (eliminando ripetizioni di codice): 4 punti - errori di programmazione
 * comportano decurtamento del punteggio complessivo
 */
...
public class Test {
	...
	public void testTripletsToSum() {
		...
		assertEquals(List.of(18, 300), sc.combine(List.of(5, 6, 7, 100, 100, 100)));
	}
	...
	public void testCountUntilZero() {
		...
		assertEquals(List.of(3, 2, 4, 2), sc.combine(List.of(1, 1, 1, 0, 2, 2, 0, 3, 3, 3, 3, 0, 5, 6)));
		assertEquals(List.of(3, 2), sc.combine(List.of(10, 10, 10, 0, 2, 3, 0)));
	}
	...
	public void testSingleReplacer() {
		...
		assertEquals(List.of("aa", "bb", "cc"), sc.combine(List.of("a", "b", "c")));
		SubsequenceCombiner<String, Integer> sc2 = this.factory.singleReplacer(s -> s.length());
		assertEquals(List.of(1, 3, 2), sc2.combine(List.of("a", "bbb", "cc")));
	}
